...
| A direct connectivity check reports `certificate verify failed` | Jenkins presents a certificate the container trust store cannot validate; MCP tool errors withhold raw TLS details | Mount the private/self-signed issuer with `jenkins.caBundle.existingSecret`; keep `jenkins.verifyTls: true` |
| `Jenkins request failed due to a transport error` | The pod cannot reach the Jenkins URL | Check `jenkins.last_transport_error` on the direct server’s `/readyz`, or the child server’s transport warnings in Minibridge logs. Verify the URL, cluster DNS, firewall allowlist, and egress policy/gateway; client-visible errors intentionally omit URLs and raw exception text |
...
| `trigger_build` reports an invalid queue Location | Jenkins or an intermediary accepted the POST without an HTTP(S) Location ending in canonical `/queue/item/<positive-id>/`, so the server cannot prove which queue item was created | Check reverse-proxy Location rewriting. A different public origin or prefix is supported because the returned `queue_url` is rebuilt from configured `JENKINS_URL`; query strings, fragments, other schemes, and non-queue routes remain refused |
...
| A Minibridge MCP session works intermittently with multiple replicas | Requests are reaching a replica that does not own the session; the direct Python HTTP endpoint is stateless | Keep the Service's `ClientIP` affinity, or provide equivalent ingress affinity. Reconnect after pod restarts |
| Minibridge logs `Backend websocket has closed` with `use of closed network connection` while pods remain ready | The bundled Minibridge in releases through 2.10.0 reported its intentional ten-second idle-session cleanup as a backend error; it did not mean that Jenkins or the pod was unhealthy | Upgrade to 2.10.1 or later. If the message is accompanied by `MCP Server exited`, restarts, probe failures, or client-visible failures, investigate those separately rather than treating them as normal cleanup |
| `jenkins.last_transport_error` is set and `last_contact_age_seconds` keeps growing | This pod cannot reach Jenkins; check egress, DNS, and the CA bundle | The pod stays ready on purpose: tool calls return a sanitized error, while this readiness field and server transport warnings identify the failure class |
...
| A read reports malformed job-list, job, build, queue, or running-build JSON | Jenkins or a plugin returned the wrong object/collection shape, so allowlist filtering and field projection could not be proven | Inspect the controller/proxy response and plugin compatibility. The server fails closed instead of returning the unfiltered payload |
| Console pagination reports invalid or missing metadata | Jenkins or an intermediary returned an invalid `X-More-Data`, a non-integer/regressive `X-Text-Size`, or claimed more data without a cursor | Inspect proxy header rewriting and Jenkins logs. Retry from the last confirmed `next_start` only after the upstream response is corrected |
