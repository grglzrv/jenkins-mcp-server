...
| A read reports malformed job-list, queue, or running-build JSON | Jenkins or a plugin returned the wrong object/collection shape, so allowlist filtering and field projection could not be proven | Inspect the controller/proxy response and plugin compatibility. The server fails closed instead of returning the unfiltered payload |
| Console pagination reports an invalid `X-Text-Size` | Jenkins or an intermediary returned a non-integer, negative, or regressive offset while claiming more data | Inspect proxy header rewriting and Jenkins logs. Retry from the last confirmed `next_start` only after the upstream response is corrected |
