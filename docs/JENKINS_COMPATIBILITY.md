...
All structured read tools use explicit Jenkins `tree` queries and documented
response projections. `list_jobs` and `get_job` exclude plugin actions and
upstream/downstream job objects. Queue and running-build tools exclude queue
actions, build-parameter values, and arbitrary executable fields.
